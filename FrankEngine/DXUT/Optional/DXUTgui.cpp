...
//--------------------------------------------------------------------------------------
void CDXUTDialog::ClearAllPressed()
{
	for (int i = 0; i < m_Controls.GetSize(); ++i)
	{
		CDXUTControl* control = m_Controls.GetAt( i );
		if (control)
			control->ClearPressed();
	}
}
...
bool CDXUTDialog::OnCycleFocus( bool bForward )
{
	...
	// fix bug if you cycle focus while pressing a control
	ClearAllPressed();

	// todo, get rid of static control focus
    if( s_pControlFocus == NULL )
    {
        if( m_Controls.GetSize() > 0 )
        {
			if( bForward )
	            pControl = m_Controls.GetAt(0);
			else
	            pControl = m_Controls.GetAt( m_Controls.GetSize() - 1 );
        }
	...
	CDXUTControl *pControlStart = pControl;
    ...
    for( int i=0; i < 0xffff; i++ )
    {
        ...
        if( pControl == s_pControlFocus || pControl == pControlStart )
            return true;
    }
...
void CDXUTEditBox::CopyToClipboard()
{
    ...
    int nLast = __max( m_nCaret, m_nSelStart );
	if (nLast - nFirst > 0)
		FrankUtil::CopyToClipboard(m_Buffer.GetBuffer() + nFirst, nLast - nFirst);
}
...
void CDXUTEditBox::PasteFromClipboard()
{
	...
	FrankUtil::PasteFromClipboard(buffer, maxPasteLength);
